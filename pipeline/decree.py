...
いるノードのことで、全国に 5,706 個、1 路線あたり約 12 個ある。大半は OSM が
尽きた場所か、県の bbox が道を切った場所であって、路線が法令上どこで始まりどこで
終わるかではない。どちらの端が起点かも、その集合は示さない。
...
表が名指しするのは地名であって座標ではない(「東京都中央区」「大阪市」)。だから
点を見つけねばならず、ここでの規則は狭い。
...
この規則を回すには二つの出どころが必要である。
  ...
  行政区域(国土数値情報 N03)   市区町村の多角形。二つの年版を、現行から順に
      読む。政令の地名は、その行が最後に改正された時点(最も新しい改正は平成
      16 年)で効力を持っていた物であり、平成の大合併はその後、およそ五分の一を
      地図から消した。清水市と中村市はデータの欠落ではなく、存在しなくなった
      市区町村である。その波の直前、2000-10-01 の年版は今もそれを描いており、
      名前が変わっても土地は動かない。
...
はたいてい国道どうしの交点であり、日本橋は六つの路線の端点である。残った物は、
その路線自身の端点の中心から最も遠い端点を選ぶ。起点と終点は路線の両端であって、
途中のどこかではないためである。

同じ市区町村で始まり終わる路線は環状(国道16号と国道302号)で、遠い側を選ぶ規則
は何も言えない。その行は地名だけを持ち、座標を持たない。
