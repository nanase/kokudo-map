...
地図が今 端点 と呼んでいる物は、その路線のアークが終わっているノードというグラフ
上の事実で、全国に 5,706 個、1 路線あたり約 12 個ある。大半は OSM が尽きた場所か
県の bbox が道を切った場所で、法令上どこで始まりどこで終わるかでも、どちらの端が
起点かでもない。

政令のほうは示している。一般国道の路線を指定する政令(昭和 40 年政令第 58 号)は
459 行の別表を持ち、列は 路線名・起点・終点・重要な経過地 である。e-Gov 法令検索
がそれを XML で配る。このスクリプトはその表を読み、端点の置き換えではなく隣に
並ぶ独立した欄として書く。

表が名指しするのは地名(「東京都中央区」「大阪市」)であって座標ではない。だから
点を見つけねばならず、規則は狭い。

    政令の起終点の座標は、その路線自身の端点のうち、政令が名指しする市区町村の
    中に在る物である。

座標が作られることは無い。必ず路線が実際に終わっているノードで、必ずその
市区町村の中にある。中に端点が無ければ、その行は地名だけを持つ。半分だけ正しい
座標が「政令上の起点」の名を着ているより、無いほうがよい。

出どころは二つある。

  行政区域(国土数値情報 N03)   市区町村の多角形。二つの年版を現行から順に読む。
      政令の地名はその行が最後に改正された時点(最も新しい改正は平成 16 年)の
      物で、平成の大合併はその後およそ五分の一を地図から消した。清水市と中村市は
      データの欠落ではなく、存在しなくなった市区町村である。その直前、2000-10-01
      の年版は今もそれを描いており、名前が変わっても土地は動かない。

  端点                          build/regions/*.meta.json。路線の端点は報告する
      地域すべてで和を取る。県の bbox は重なるので、同じノードが隣り合う二県から
      報告される。

同点は二段で解く。まず、別の国道もそこで終わっている端点が勝つ。法令上の起終点
はたいてい国道どうしの交点で、日本橋は六つの路線の端点である。残った物は、その
路線の端点の中心から最も遠い端点を選ぶ。起点と終点は路線の両端だからである。
同じ市区町村で始まり終わる環状の路線(国道16号、国道302号)では遠い側を選ぶ規則
が効かないので、その行は地名だけを持つ。
...
# 一般国道の路線を指定する政令(昭和40年政令第58号)。e-Gov 法令検索 API v1 は
...
# 誰も --refresh を覚えていなくても、改正の頻度に近い所へ落ち着く。
...
class Gazetteer:
    ...
    索引を組むのに読むのは DBF だけである(全国で 1,900 行の文字列)。市区町村の
    多角形は、誰かが求めたときに初めて SHP から読む。
    """
    ...
    def _name(self, idx: int, pref: str, gun: str, muni: str, ward: str) -> None:
        ...
        表の書き方は元から揃っていない。大阪市は単独で、東京都中央区は都を
        付けて、熊本県鹿本郡植木町は郡まで付けて書く。そのどれもが同じ輪郭を
        指さねばならない。
        """
    ...
    def lookup(self, place: str) -> tuple[str, set[int]] | None:
        ...
        「岩国市麻里布町一丁目」の丁目は N03 が描くどの単位よりも細かい。
        市区町村になる最も長い接頭辞が、正直に降りられる限界である。
        """
...
def meeting_counts(routes: dict[int, list]) -> dict[int, dict]:
    ...
    まとめる距離は、地図が起終点の共有に既に使っているのと同じである。1 つの
    交差点は複数のノードでできているので、そこで出会う端どうしはぴたりとは
    重ならない。
    """
    lat_cell = TERMINI_CLUSTER_M / 111_320  # degrees of latitude
    # 経度 1 度の長さは北へ行くほど短いので、同じ度数でも宗谷のセルは八重山の
    # セルより狭い(105 m に対して 150 m)。すると 150 m 離れた二つの端点が二つ隣
    # のセルに落ち、隣を 1 つしか見ない下の走査はそれを見落とす。経度側のセルの
    # 大きさを、データの中で最も北にある端点で決めておけば、どのセルも
    # TERMINI_CLUSTER_M 以上の幅を保つ。
    ...
    for row in rows:
        ...
        for side in ("start", "end"):
            ...
            if hit is None:
                ...
                # 環状の路線。起点も終点も同じ市区町村なので、遠い側を採用する
                # 規則が二つの端点を区別できない。
                cell["how"] = "ring"
            else:
                g, _, idxs = hit
