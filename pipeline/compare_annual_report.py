...
ぶんは残りとして出す。
  ...
  工事中・未開通  OSM の `construction` は、まだ無い道を造ることだけでなく、
                  ...
                  後者は二重計上で、二つは同じ番号の開通済みの道が並んで走って
                  いるかどうかで見分ける。
  ...
  旧道            両方とも旧道を数える(台帳は実延長の中に、専用の欄として持つ)
                  ので、分類の差であって延長の差ではない。台帳の旧道の欄の隣に
                  並べて報告し、計算からは外す。

このスクリプトは都道府県別の突き合わせをしない。長らくそれはできなかった。地域が
矩形で隣県が食み込むので、県ごとに数えたつもりの値は bbox を測った値にしか
ならなかったからである。全国計は影響を受けない。どの bbox に入っていようと、
way は id で一度だけ数えるからである。

その障害は取り除いてある。way は行政区域の面で決めた所属都道府県 `pref` を持つ
(prefectures.py)ので、県ごとに数えられる。それでも全国計だけを見るのは、国道の
番号が全国で一意だからである。県別に割っても、全国計より細かい問いに答えるだけ
で、番号の取り違えは見つからない。県別でしか答えられないのは都道府県道のほうで、
そちらは compare_annual_report_pref.py が県ごとに突き合わせている。
