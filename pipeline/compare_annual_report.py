...
二つは別々の物を数えている。答えの大半はそこにある。
  ...
  台帳    路線の中心線を数える。上下線が分かれた道も 1 本ぶんで、ランプは路線の
          延長ではない。認定は道ができる前に済むので、開通していない道にも延長
          がある(未供用)。
  地図    OSM の way を数える。上下線が分かれた道は way が 2 本あり、どちらも
          路線番号を持つので両方を描く。そのための地図である。

問いはどちらが正しいかではなく、地図の km のうち台帳が数えていないのはどれで
どれだけか、である。原因はどれも実測し、説明しきれないぶんは残りとして出す。

  海上区間        台帳の未供用には 1,953 km の海が入る。地図が持つのはルート
                  リレーションが含む航路の way だけである。
  工事中・未開通  OSM の `construction` は、まだ無い道を造ることだけでなく、
                  台帳にも載っている道を造り直すことも指す。後者は二重計上で、
                  同じ番号の開通済みの道が並んで走っているかで見分ける。
  ランプ・連結路  highway=*_link。附属物であって路線の延長ではない。
  上下線分離      同じ番号の way が並んで走り、どちらも一方通行である way。
                  車道が二本で道は一本である。測り方は paired_fraction を参照。
                  台帳は 1 本ぶん、地図は 2 本ぶんなので、並走の長さの半分が
                  超過ぶんになる。
  旧道            両方とも数える(台帳は実延長の中の専用の欄)ので、分類の差で
                  あって延長の差ではない。台帳の旧道の欄の隣に並べ、計算からは
                  外す。

都道府県別の突き合わせはしない。かつては地域が矩形で隣県が食み込むのでできず、
全国計だけが way を id で一度だけ数えて影響を免れていた。今は way が行政区域の
面で決めた所属都道府県 `pref` を持つ(prefectures.py)が、それでも全国計だけを
見るのは、国道の番号が全国で一意だからである。県別に割っても番号の取り違えは
見つからない。県別でしか答えられないのは都道府県道で、そちらは
compare_annual_report_pref.py が県ごとに突き合わせる。
...
# way に沿って 25 m ごとに標本を取る。way は短い(70,376 km を 151,114 本で割ると
# 1 本 466 m)うえ、車道の対が way の途中で始まったり終わったりすることはまず
# 無いので、これより細かく取っても時間がかかるだけで値は動かない。
SAMPLE_M = 25.0
...
def load_region(region: str) -> list[dict]:
    ...
    方向の尺度が数 % ずれるが、40 m のしきい値に対しては問題にならない。
    """
...
def paired_fraction(arc: dict, ai: int, arcs: list[dict], grid, cell: float,
    ...
    """このアークのうち、同じ番号の way が並んでいる長さを m で、区分ごとに
    返す。

    キーは (このアークの区分, 隣の way の群, 両方とも一方通行か) である。後から
    原因を見分けられるのはこのキーのおかげである。開通済みの車道が二本並ぶのと、
    置き換えられる道の隣で造り直しが進むのとは別のことである。アーク自身の区分
    を群ではなくそのまま残すのは、二重計上のうちどれだけが自動車専用道路(車道が
    二本あるのが普通の区分)なのかを報告で示すためである。
    """
...
def measure(reach: float) -> dict:
    ...
    for region in REGIONS:
        ...
        # 台帳にとって旧道はそれ自体で 1 本の道であり(実延長の旧道の欄)、迂回
        # した道の複製ではない。だから対にもならず、隣の相手にもならない。ランプ
        # を外すのは裏返しの理由で、ランプ・連結路として既に丸ごと数えてある。
...
def take_distance(args: list[str]) -> tuple[float, list[str]]:
    ...
    都道府県道の側(compare_annual_report_pref.py)も同じフラグを同じ意味で取る
    ので、読み取りはここに一つだけ置く。値を検査するのは、`--distance` を末尾に
    書いたときや `--distance --no-pairing` と書いたときに、添字の失敗ではなく
    使い方の誤りとして落とすためである。extract_pbf.take と同じ形である。
    """
...
def one_timestamp(stamps: set[str]) -> str:
    ...
    別々の切り出しから作った物が混ざると全国計は意味を失うが、合計そのものは
    それを示さない。だから混ざった生成物は隠さずに名指しする。読む側は地域の
    meta と build/survey の二つあるので、述べ方はここに一つだけ置く。
    """
...
def row(label: str, ledger: float | None, map_km: float | None, width: int = 28) -> str:
    """突き合わせの 1 行。台帳の値、地図の値、その差である。

    どちらの側も None になりうる。徒歩道には突き合わせる欄が台帳に無く、そう
    述べることは 0 と述べることとは違う。
...
def main() -> None:
    ...
    # 道が二本の車道になるのはそれが理由である。まったく同じ問いではない
    # (中央帯は幅を持つ構造物であり、OSM は塗り分けただけの分離帯でも way を
    # 分ける。しかも表 8 のこの欄は、自動車専用道路のアークが
    # 持つ高速自動車国道との重用を外している)が、二つは近い値に
    # 落ち着くはずであり、実際そうなる。
    print("\n裏取り")
