...
# ///
"""国土数値情報(N03, 行政区域)の在り処と、それを取ってくる手立て。

この配布物は二人が読む。decree.py は市区町村の輪郭を、政令の起終点に座標を
当てるために読む。prefectures.py は都道府県の輪郭を、way の所属を決めるために
読む。二人が同じ省庁の同じ配布物を同じ場所へ落とすので、どこから取るかを二箇所
に書けば片方が暗黙のうちに古くなる。ここが一箇所である。

`cached` は N03 だけの物ではない。decree.py は e-Gov の法令 XML も同じ関数で
取る。取ってきた物をディスクに残して次から読む、という振る舞いは、相手が誰でも
変わらないためである。
"""
