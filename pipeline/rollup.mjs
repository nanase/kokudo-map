 ...
 * 一覧も重用のランキングも選択の合計も、あちらでは足せない。ここで足して meta と
 * して配る。
 *
 ...
 * bbox, geometry}` の形である。`refs` は区切り文字で囲んだ鍵、`refs_list` は
 * その中身の並びである。
 */
 ...
 *  足すと共有部分を二重に数える——18 号と 117 号を持つアークは両方に属する。それは
 *  この地図が隠すのをやめさせたい数そのものである。
 *
 ...
 *  名指しするのは 1 つか 2 つである。0 の五つを書き並べれば、何も述べないまま表が
 *  三倍になる。
 *
 *  `extra` は、その路線の格に固有の欄を 1 行ごとに足す。都道府県道は主要地方道か
 *  どうかをここで持つ——境目の値を持っているのは判定(build_prefectural.py)なので、
 *  JS 側が番号から決め直すと、同じ問いに二箇所で答えることになる。
 */
 ...
 *
 * 節点は座標そのもので同定する。同じ OSM 節点から出た座標は判定が同じ桁
 * (小数第 6 位、約 10 cm)で丸めているので、文字列として一致する。立体交差は節点を
 * 共有しないので、ここには出ない——曲がれない交差は交差ではない。
 *
 ...
 *
 * `compare` は路線の並べ方である。組の中も、出来上がった並びも、これで揃える。
 */
