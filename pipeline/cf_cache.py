...
UNWRITABLE_ZONE_SETTINGS = {
    # 書けるが当てない。API は編集を許す(Cloudflare も editable=true と答える)。
    # これは設定ではなく一時的な状態である。誰かが調査のために有効にしている
    # 最中に書き戻せば、断りなく無効化してしまう。逆に記録が古ければ、誰も
    # 望んでいないのに有効化してしまう。どちらもこちらが決めてよいことではない。
    "development_mode": "一時的な状態であって設定ではないため",
    # そもそも書けない。このゾーンの契約では読み取り専用(editable=false)で、
    # PATCH は code 1015(Not allowed to edit zone setting)を返す。同じ値でも
    # 拒まれる。試みると、そこまでの書き込みだけが済んだ状態で落ちる。
