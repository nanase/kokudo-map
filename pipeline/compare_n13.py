...
audit.py は路線の連なりの切れ目を見つけるが、OSM のキャッシュに痕跡がある道しか
推論できない。OSM に丸ごと無い道を見つけられるのは独立した出どころだけである
(TRIAGE.md)。N13 は地理院地図と同じ電子国土基本図に由来し、国内の一般国道を
すべて覆う。ただし路線番号は持たず、道路分類(国道 / 都道府県道 / 市区町村道等 /
…)しか持たない。だから、どの番号が欠けているかは言えず、何かがどこに在るか
だけを言える。

突き合わせは二つあり、同じ問いを逆向きから見ている。

  gap        近くにこちらの物が無い N13 の国道の線分。「その道は OSM にそもそも
             無い」の候補で、audit.py が届かない TRIAGE.md の場合である。
  orphan     近くに N13 の国道の線分が無い、こちらの旧道フラグ付きのアーク。
             「地理院地図は既に指定解除しており、旧道フラグが古い」の候補で
             ある。手掛かりは、旧道のアークが N13 の線の近くに在るかどうかで
             ある。N13 が「旧道である」と同意するかは見ても意味が無い。N13 は
             現道と旧道を区別せず、法令上の旧道は指定解除まで現道と同じ
             道路分類=国道 を持つ。それを見ると、正しくタグ付けされた旧道が
             残らず「食い違い」として挙がる。

orphan 側は、1 点ではなく被覆率で評価する(issue #27)。アークを SAMPLE_INTERVAL_M
ごとに取り直し、N13 から ORPHAN_THRESHOLD_M 以内にある標本の割合を被覆率と
する。中点 1 点(旧来の規則)も端点の頂点(旧道は現道と繋がり直すので、作りから
して N13 の近くに在る)も、「この道のどれだけが今も N13 に裏付けられているか」
とは別のことを示す。繋がったアーク(1 本の道が短い way に分かれている場合や、
重複排除の前で県境の写しに分かれている場合)は 1 つのクラスタにまとめ、長さで
重み付けした 1 つの割合を持たせる(cluster_former_arcs。gap 側の cluster_gaps と
cluster_by_endpoint を共有する)。仕分けを way ごとではなく道ごとに行うためで
ある。`region all` はさらに、同じ way id を評価の前に重複排除する
(national_orphan_report)。

被覆率の低いクラスタが示すのは「近くに国道があまり無い」ことだけで、代わりに
N13 が何を描いているかは示さない。そこで、クラスタの標本の点に最も近い N13 の
線を道路分類を問わず引き、その分類と距離も報告する。国道でないと分かる分類
(都道府県道 / 市区町村道等 / 高速自動車国道等 / その他)が近くに在ることは、
単に無いことより強い手掛かりである。こちらの道が走るまさにその場所に、N13 が
国道でない物を描いているからである。指定解除(道が消されるのではなく、下位の
区分へ移されること)は、この出どころからはそう見える。不明 は数えない。裏付ける
物を何も主張していないためである。

どちらの向きも同じ格子を使う。地域のアークは短い OSM の way で、N13 のレコードは
さらに短い(属性が変わるたびに切られている)ので、最近傍の頂点ではなく最近傍の
線分までの距離を、局所的な正距円筒で測る。頂点だけを見ると、頂点の間隔が
そのまま偽の食い違いに読める。

距離だけでは証明にならない。隙間は、本物の欠落かもしれないし、OSM のタグによる
除外(`why`。audit.py の除外の理由をそのまま使う)かもしれないし、同じ線を二つの
出どころが別々に描いただけかもしれない。どれなのか、どの路線番号なのかは、
地理院地図を開いた人が決める。TRIAGE.md の「N13 は路線番号を持たないので…
ここが人の出番です」はその部分を指す。確認済みのクラスタだけは、指定解除が
起きたこと自体を人手なしで言える。かつて何号だったかは、今も人が決める。この
確認は apply_n13.py が `revoked` として生成データに書く(issue #9)。
...
# 404 になることを手で確認したメッシュの符号。陸が無く、N13 は道路しか
# 覆わないので、KSJ はその shapefile を配っていない。2026-08-22 に regions.py の
# 47 都道府県すべてに触れて出た 404 の全部である(相異なるメッシュ 272 個のうち
# 125 個)。実行時のログの grep だけでなく、ディスクの build/n13/ とも
# 突き合わせてある(メッシュのキャッシュはあるのに展開した .shp のディレクトリが
# 無いメッシュは、下の 404 の分岐を通ってしかそうならない)。最初に手で集めた
# 一覧は、確認の実行がファイルへ流れていなかったメッシュを 5 つ
# 取り落としていた。この集合に無いメッシュの 404 は同類だと仮定しない
# (ensure_mesh)。新しい符号を加えてよいかは、KSJ の URL が 404 を返し、地図の
# 上で外洋であることを手で見るしかない。これから出る 404 すべてへ、見ないまま
# 理屈を広げてはいけない。
...
@dataclass
class Mesh:
    """そのメッシュの全レコード(国道だけでなく全分類)を、bbox で切らずに持つ。

    座標は Python のオブジェクトの木ではなく numpy の配列 1 本である。1 点を
    tuple で持つと、生の 16 バイトが何倍にも膨らむ。最大のメッシュ 5339 は
    190 万レコード、749 万点で、生の座標 114 MB に対して常駐 2,323 MB(20.3 倍)
    だった。issue #103 が測ったピーク 2,466 MB の正体である。(点数, 2) の
    float64 の配列と区切りの添字で持てば、ディスクで 124 MB、mmap で開くので
    常駐にはほとんど乗らない。

    読み取り専用の mmap には、常駐を増やさない以上の意味がある。同時に走る何本
    ものプロセスが同じ物理ページを共有するので、県ごとの段を並列にしても、N13 の
    ぶんのメモリが並列度に比例して増えない。

    pts     (点数, 2) float64。(緯度, 経度) をレコードの順に繋げてある
    starts  (レコード数 + 1,) int32。pts への区切りで、レコード i の座標は
    ...
    def segments(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        pts は全レコードを繋げた 1 本なので、隣り合う 2 点が必ず 1 本の線分とは
        限らない。レコード k の最初の点の手前の線分は、前のレコードの最後の点と
        結んでしまう。starts がその境目を持つので、そこだけ落とす。

        分類は点ごとに広げてから 1 つずらす。線分 j はレコード内に収まるので
        (上で境目を落としてある)、始点の分類がその線分の分類である。広げた
        物をそのまま持つのは、線分 1 本の分類を引くたびに starts を
        二分探索するより安いからである。最大のメッシュでは memmap への
        searchsorted に 1 本あたり 3.6 ms かかっていた。線分ごとに
        2 バイトなので、最大のメッシュでも 12 MB に収まる。
        """
        starts = np.asarray(self.starts)
        n = len(self.pts)
...
def pack_mesh(mesh: str, refresh: bool) -> None:
    ...
    キャッシュはメッシュごとに 1 つで全分類を覆う。高く付くのは shapefile の解析
    であり、絞った後の物をキーにした二つ目のキャッシュを持つと、「ここに在る国道
    以外は何か」に答えるために同じ shapefile を解析し直すことになる。

    キャッシュはメッシュの生の、絞っていないレコードを保ち、キーはメッシュだけで
    ある。1 次メッシュは二つの地域の境をまたぐことが珍しくない(矩形が隣県へ食み
    込むことは regions.py)。書き込み時に絞ると、共有するメッシュに二番目に触れた
    地域が、最初の地域の bbox で絞った物を暗黙のうちに使い回す。絞り込み(bbox も
    分類も)は呼ぶ側に任せる。Mesh.lines を参照。

    座標は array("d") へ積む。Python のリストへ float を積むと、1 点あたり float
    オブジェクト 24 バイトと参照 8 バイトが残る。array は生の倍精度をそのまま
    並べるので、解析中の山も座標のバイト数のままである。
    """
    ...
    # 込みの途中で止まると(Ctrl-C、メモリ不足による強制終了)、途中までのファイル
    # が残り、次の実行は解析し直さずに np.load で落ちる。同じディレクトリなら
    # 名前の付け替えが 1 つのファイルシステムの中で済み、不可分になる。
    #
    # 3 本のあいだの取り決めは 1 つである。pts が在ることが、3 本が同じ組で
    # 揃っている印である。load_mesh はその 1 本の有無だけを見る。だから pts は
    # 最後に置き、書き直す前にまず消す。
    #
    # 先に消す理由は --refresh である。既に 3 本ある状態で書き直すと、pts が
    # 古いまま starts と rdctg が先に新しくなる瞬間がある。そこで止まると、次の
    # 実行は「pts が在る」を見て、新しい starts と古い pts を組にして開く。
    # 座標の区切りが中身とずれた物を暗黙のうちに使うことになる(CodeRabbit の
    # レビュー)。先に消しておけば、その瞬間に残るのは「pts が無い」、つまり
    # 作り直す状態だけである。
...
def nearest_classified_in_mesh(points: list[tuple[float, float]], mesh_data: Mesh
    ...
    セルの半径で探すやり方(以前の版がそうで、このファイルの他の最近傍線分の
    探索は今も格子と CELL を使う)が安全なのは、呼ぶ側がしきい値と比べていて、
    しきい値が探索の広がりに比べて十分小さいときだけである。他の格子の
    参照はどれもそうだが(数 km の広がりに対して 100 m のしきい値)、
    classify_beneath の「直下」はそうではない。あちらは本当に最も近い線までの
    距離を、CONFIRM_THRESHOLD_M をはるかに超えていても報告する。どれだけ大きく
    取った半径も、分類済みの道がどこまでまばらになりうるかの当て推量であり、
    CodeRabbit のレビューは、その当て推量が二度(約 1 km と約 4 km)、まだ本物の
    線分を見落としうると指摘した。線分を全部走査すれば、当て推量そのものが
    無くなる。

    走査は numpy で回す。Python のループで 1 本ずつ測ると、最大のメッシュの
    550 万本に数秒かかり、(クラスタ, メッシュ) の対ごとにそれを払う。福島県
    1 県の apply_n13 が 301 秒かかっていたのはここである(issue #103)。

    問い合わせ点は 1 つずつではなく、そのメッシュを見たい物をまとめて受ける。
    繰り返しの順序が「塊が外、点が内」なのはそのためで、1 つの塊とその一時配列が
    CPU のキャッシュに載っているあいだに、その塊に対する全部の点を片付ける。点
    ごとにメッシュを頭から舐め直すと、同じ数の演算に対してメモリを何倍も往復
    する。茨城県は 9 メッシュに対して 34 クラスタを問うので、この順序だけで
    メッシュを読む回数が 233 回から 9 回になる。

    ただし返す距離は numpy の値ではなく point_segment_distance_m の値である。
    numpy が決めるのは「どの線分が最も近いか」だけで、決まった 1 本を本物の
    関数へ渡し直す。だから numpy の側は括り方を変えて演算を減らしてよい。最後の
    1 ビットが動いても、選ばれ方が変わるのは 1 ULP 差で並んだ二本のどちらを
    採用するかという場面だけで、返る値はどちらでも本物の関数の出力である。しきい
    値と比べる値も報告に出る値も、このファイルが距離だと定義した式の出力になる。
    写した式ではない。

    返すのは points と同じ長さの一覧で、各要素は次の対である。
    (nearest_any, nearest_confirmable)。意味は classify_beneath を参照。どちらも
    (距離, rdCtg) か None である。
    """
...
def classify_clusters_beneath(clusters: list[dict], refresh: bool,
    ...
    付ける。値は、クラスタの標本の点に最も近い、分類を問わない N13 の線から
    作る。

    1 メッシュの全分類のレコードは、国道だけの 30〜50 倍になる。メッシュあたり
    74,000〜137,000 件に対し、国道は 2,000〜3,000 件である(issue #28)。かつて
    Python のオブジェクトへ起こしていた頃は、クラスタが跨りうるメッシュを
    全部まとめるとメモリを使い果たしたので、1 メッシュぶんを読んでは捨てていた。
    今は Mesh が mmap で開くので、開く費用そのものはほとんど無い。それでも
    メッシュごとに繰り返すのは、同じメッシュを何度も開き直さないためである。
    分類を見る処理を coverage_ratio と同じ繰り返しに置けないことは変わらない
    (あちらはメッシュごとの国道だけの部分集合を 1 つの格子へ読み込む。そちらは
    十分小さい。load_kokudo_raw を呼ぶ側)。

    道はメッシュの境界ごとに切られているので、境界から CONFIRM_THRESHOLD_M
    以内の標本の点は、本当に最も近い N13 の線が隣のメッシュに在ることがある。
    標本を含むメッシュだけを見た以前の版は、縁の近くで確認済みを数え落としていた
    (CodeRabbit のレビュー)。代わりに、クラスタごとに自分のメッシュと 8 近傍
    (neighbor_mesh_codes)を見て、最も良い一致を 1 つ残す。見るのは known_meshes
    (呼ぶ側が解決済みのメッシュの集合。地域の `meshes` か全国の `all_meshes`)に
    在る近傍だけである。県の bbox は大きく重なるので、本物のクラスタが近づきうる
    メッシュはその集合に入っている。絞っても失う物は無く、ensure_mesh が一度も
    検証していないメッシュへの要求を避けられる。そこで本当に新しいメッシュが
    404 を返すことは、止まって手で確かめる理由であって、当て推量で引き当ててよい
    物ではない。
    """
    for c in clusters:
        c["_nearest_any"] = None
...
def coverage_ratio(coords: list[tuple[float, float]], grid,
    ...
    アークの頂点でも中点 1 点(旧来の規則)でもない。旧道のアークは両端で現道と
    繋がり直すので、その端点は、道が今も N13 に裏付けられているかに関わらず
    threshold_m 以内に入る。中点だけを見れば、一部しか裏付けの残っていない道の
    どこにでも落ちうる。全長に沿って標本を取ることで、この割合は「その道の
    どれだけか」になる。

    min_dist は、どの標本が N13 に最も近づいたかであり、threshold_m を超えていて
    も返す。issue #27 の PR への CodeRabbit のレビューが、割合 0 は「近くに N13
    が無い」を意味しないことを捉えた。どの標本からも threshold_m のすぐ外に在る
    N13 の線は、被覆率 0% だが「N13 なし」ではない。それを意味するのは min_dist
    が None のとき、つまり nearest_segment の探索半径の中でどの標本も何も
    見つけられなかったときだけである。
    """
