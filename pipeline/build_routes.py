...
# まだ造っている道についての同じ格。かつては `primary` も入っており、primary を
# 締め出す規則の唯一の裏口だった。長野県ではリレーションに属さない primary の
# way 3,305 本が数字だけの `ref` を持ち、国道 と名乗る物は 1 本も無い(RULES.md
# が primary を外す理由)。`construction=primary` はその一群を通し、北海道道39号
# 奥尻島線が 292 km 離れた所で国道39号に、京都府の山手幹線と岩屋バイパスが
# 国道2号になった。正しく入ったのは全国で 5 アーク(栗東水口道路、国道1号の
# 1.6 km)だけで、それは開通してタグが付け直されれば戻る。
...
# 唯一の集合でもある。extract_pbf.py は書き写さず import するので、新しいタグを
# 読む規則はここを広げねばならない。さもないとキャッシュからそのタグが暗黙の
# うちに欠ける。
#
# `oneway` だけは例外で、ここの規則は読まない。読むのは compare_annual_report.py
# で、上下線分離の二つの車道(OSM では way 2 本、台帳では 1 本)と、並んで走る
# 側道とを見分けるのに使う。切り出しが運ぶ物だからここに並べてある。
TAGS_USED = frozenset({
...
# way/152895667(長野・静岡県境の 19 m の橋)は `name` を持たず、
# `old_name=国道152号`、`highway=residential`、`route=hiking`・`tourism=yes`
# だけを持つ。OSM は「今はハイキングルートである」を 旧道・廃道 の語ではなく OSM
# の書き方で記録している。しかも今も国道152号のリレーションに載っている(規則 a
# は古い所属と生きた所属を区別しない)ので、これが無ければふつうの国道として
# 描かれた。`historic:highway` は「かつてこの格の道だった」を述べる OSM の
# 書き方で、名前とは独立である。全国で、名前で旧道と述べずにこれを持つ way は
# 1 本である(CASES.md 21)。
...
def resolve_competing_claims(competing_relations: list[dict]) -> dict[int, set[int]]:
    ...
    way が県道のリレーションに載っている理由は、その way の指定と無関係なことが
    ある。広島南道路(国道2号)は今も広島県道243号広島港線のメンバーである。
    リレーションが主張する番号を way ごとに記録しておけば、下の裏取りは、載って
    いること自体を失格の理由にせず、番号と way 自身の主張とを比べられる。

    大半は都道府県道のリレーションだが、事業者が自分の路線番号を名乗る体系も
    同じ形の主張をする。way/560259106(首都高速都心環状線、`ref=1`)は
    `network=首都高速道路`、`ref=1` のリレーションに載っている(CASES.md 20)。
    """
...
# 国道の海上区間。下に道が無いまま指定だけが水面を渡る。大半は `route=ferry`
# を持たず、全国で 20 アーク、1,390 km の外洋が車道として実線で描かれていた
# (航路としてタグ付けされていたのは 2 アーク、63 km)。走れそうに見える 295 km
# の直線こそ破線の海上国道の層が防ぐ取り違えなので、海上区間 の語を証拠として
# 読む。
#
# その語は名前にあるとは限らない。海上区間 と述べる 34 本のうち 20 本は `name`
# に、14 本は `description` か `note` にしか書かず、両者は別の路線である。
# 名前だけを読んでいたあいだ、16・28・30・42・57・259・317・324 号が東京湾・
# 明石海峡・備讃瀬戸・伊勢湾・有明海を実線で渡っていた。
SEA_SECTION = re.compile(r"海上区間")

# 指定はされているが道がまだ無い区間。ルートリレーションを繋ぐために投稿者が
# 引いた直線で、走れる物ではない。全国で 86 アーク、173.3 km(`proposed` が 76、
# `planned` が 10)。最長は国道360号の 16.4 km(白山白川郷ホワイトロードが
# 代替路)、次いで国道274号の 16.1 km(`description=国道274号不通区間`)。`road`
# のままだと CASES.md 8 と 12 の「何も無い所の直線が走れる道に読める」誤りが陸で
# 起きる。
#
# 海上区間の判定より後に置く。海上区間 と述べる 34 本のうち 32 本が
# `highway=planned` を持つ(CASES.md 12)ので、`hw` を先に見るとあの修正が帳消しに
# なる。
UNOPENED_HIGHWAYS = {"planned", "proposed"}

# 同じことを逆向きにタグ付けした形。`highway` を持たず `proposed=trunk` や
# `planned=*` だけを持つ way で、「造ればこの格になる」であって道ではない。
# way/743758644(岐阜県、layer=-5)は国道257号のリレーションのメンバーだが、
# 下にトンネルも道も無く、峠を貫く直線があるだけである。`hw` が `None` なので
# 上の UNOPENED_HIGHWAYS をすり抜けて `road` に読めた。国道リレーションが保証
# する way で該当するのは全国でこの 1 本である(CASES.md 22)。
UNOPENED_TAGS = {"proposed", "planned"}

# 高速道路として指定された国道。平面交差が無く `highway=motorway` で描かれ、
# 国道番号のうえに高速道路の路線番号を持つ(第二神明道路は `ref=E93;2`、東海環状
# 自動車道は `ref=C3;475`)。この `ref` の形が、extract_pbf.py が `tokens` を
# import するようになった不具合の原因だった。走れる本物の車道なので破線の区分
# には加えず、凡例と表示の切り替えを別に持つ。
#
...
# ルートリレーションには記入の誤りで道以外も載る。レッドバロン川口南(小売店の
# 建物の輪郭)は `highway` を持たないまま国道122号のリレーションに載り、指定
# されたアークとして描かれていた。全国で `highway` を持たないメンバーは 124 件
# あるが、閉じた環(建物の外形線、トンネル・橋の輪郭)は数件で、残りは `国道352号`
# のような名前を持つ道の形なので残す。`route=ferry` を外すのは、海上区間の way
# が `highway` を持たないのは正当だからである。
...
#
# 格だけでは決められない。規則 b が名前だけで認める全国 10,810 本のうち
# `residential` は 33 本あり、他の 32 本は正当な旧道・側道か、`ref` を持ち車が
# 通る本物の国道である。自動車が通れないことまで絞ると、当たるのはこの 1 本だけ
# である。
