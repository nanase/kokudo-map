...
判定は二つに分かれ、それぞれ別の証拠を必要とする。
  ...
  何号か             次の和集合である。
      ...
      - way 自身の `ref` を ; で区切ったトークン。こちらも保証されている番号に
        限り、かつ、その way について競合するリレーションが同じ番号を主張して
        いない物だけを採用する。競合するリレーションは大半が都道府県道だが、
        事業者が自分の路線番号を名乗る体系(首都高速道路 など)もまったく同じ形の
...
def main() -> None:
    ...
    for wid, w in ways.items():
        ...
        if from_rel or from_name:
            ...
            # である。番号を解決できないリレーションに属しているか、どのリレー
            # ションにも属さず国道の格で描かれているかのどちらかで、その番号を
            # 主張する競合する路線も無い。バイパスの場合がこれである。
            source = "tag"
        else:
            dropped.append(wid)
            continue
