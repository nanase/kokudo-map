...
残るのはリレーションと `ref` の二つで、その和を採用する。番号は県の中でしか
一意でないので、路線の同一性は (県, 番号) の組である。所属県は prefectures.py が
N03 の面で決めた物をそのまま使う(#97)。

読むのは build/survey/{region}.json である。pbf を一度読んで都道府県道になりうる
way を県ごとに落とした物で、国道にとっての build/cache に当たる。あちらが残す
集合はここが採用する集合より意図して広い。`_link` と工事中を残すのは、年報との
差の内訳を測るためである(survey_prefectural.py)。判定はここだけが持つ。
...
# 都道府県道が持ちうる路線番号の上限。全国で測ると、都道府県道の
# ルートリレーションが保証する番号の最大値は北海道の 1,180 で、次に現れる番号は
# 3,504 まで飛ぶ。1,181 から 3,503 のあいだに (県, 番号) の組は 1 つも無い。
# 空いているのはここだけなので、境目はここにしか置けない。
#
...
#
# 政令指定都市の市道が、県道と同じ書式で自分の番号を名乗っている形である。国道側
# で都市高速が `ref=4` を名乗るのと同じことが、一段下の格で起きている。
MAX_REF = 1199
...
#
# 分けるのに使うのは番号のほうである。格は道の作りを示すのであって、路線の格付け
# ではない。しかも格で分けると、東京都で 633.8 km の一般都道が主要地方道になる。
# 県ごとに境目を実測して動かす案は、沖縄県でどの測り方でも壊れる。四つの案の
# 突き合わせは PREFECTURAL.md にある。
#
# 42 県では 1〜100 が主要地方道である。残る 5 都道県では、政令の指定が番号の
# 付け方から外れる。外れているのは番号の読み方ではなく、その都道県の番号の振り方
# そのものなので、都道県ごとに書き下す。
#
...
#
# ここは法令の写しであって、実データを測って決めた値ではない。出どころは
# Wikipedia「主要地方道」の路線番号の節と、各都道県の県道の一覧である。測って
# 動かしてはならない。
#
# 範囲は出典が述べるとおりに写す。北海道は「一般道道は 200 番台以降」と
# 境目のほうが述べられているので 199 まで採用する。東京都は「301〜319番が
# 特例都道である主要地方道」と番号そのもので述べられているので、300 番台をすべて
# 採用するのではなく 319 で切る。次の特例都道の帯(401〜484)は一般都道なので、
# あいだの 320〜399 を主要地方道と読む根拠はどこにも無い。
#
...
def national_name_numbers(name: str | None) -> set[int]:
    ...
    全国で測ると、候補のうち名前が `国道N号` と述べる way は 41 本あり、その番号
    が `ref` とも一致するのは 3 本(0.17 km)である。うち 2 本は `primary_link`
    なので `admits_by_tag` が先に外す。この規則でしか落ちないのは静岡県の 1 本、
    0.03 km である。

    残る 38 本(4.9 km)は `name=石川県道22号金沢小松線;国道159号` のように別の
    番号を述べており、国道と県道の正当な重用である。番号を見ずに「名前が
    国道だから」で外すと、この 38 本を壊す。

    国道のリレーションに載っていることは、この検査には使えない。候補のうち国道の
    リレーションが番号を与える way は 224 本あるが、番号が一致するのは
    1 本だけで、残る 223 本(92.6 km)は青森県道3号 弘前岳鰺ケ沢線のように国道と
    重用する本物の県道である。CASES.md 18 が国道側で学んだのと同じことが、こちら
    側でも起きる。「国道のリレーションに乗っている」ことと「国道として同じ番号を
    名乗っている」ことは別の情報である。
    """
...
def admits_by_tag(doc: dict) -> bool:
    ...
    は primary が 128,139、secondary が 152,701 に対して tertiary は 2,470 で
    二桁少なく、国道が primary を締め出したのと同じ理由で外せる。

    `_link` は格として認めない。ランプは台帳の路線延長ではないからである。
    国道側の `is_national_grade` が `trunk_link` を認めないのと同じ扱いで、
    リレーションが抱えるランプは規則 a でそのまま入る。全国の `_link` 2,770 本・
    424.0 km のうち 1,429 本・215.1 km がそちらで入り、残る 1,341 本・208.9 km
    がここで落ちる。

    工事中は認める。国道側が `construction=primary` を締め出したのは、それが
    数字の `ref` を持つ県道を国道の裏取りへ流し込む裏口だったからである(北海道道
    39号が国道39号になった)。こちら側では、それはまさに拾いたい物である。
    """
...
def refs_key(region: str, refs: list[int]) -> str:
    ...
    国道の `,18,` に当たる物だが、県道の番号は県の中でしか一意でないので、県の
    名前を番号に貼り付ける。県道18号は 47 本あり、`,18,` ではどの 18 号かを
    示せない。

    貼り付ける県は way の所属県そのもの(`pref` 属性と同じ値)である。同じことを
    二度書いているように見えるが、片方は way の属性、もう片方は路線の同一性で、
    必要な形が違う。この形なら、画面の絞り込みは国道と同じ 1 つの
    式のままである。
    ...
    県は符号(13)ではなく地域名(tokyo)で書く。この repo が県を名指しするときの
    言葉は regions.py の地域名で、build/survey のファイル名も
    web/data/regions.json も `pref` 属性もそれである。ここだけ二桁の符号を
    持ち込むと、県の名前が二通りになる。
    """
