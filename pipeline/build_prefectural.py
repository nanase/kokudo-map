...
都道府県道では名前が使えず、裏取りが効かない。

  名前         OSM の `name` は路線名ではなく場所の呼び名である。県内の
               (番号 -> 名前)13,223 組のうち 5,050 組が複数の名前を持ち、中身は
               「環状1号線」「1条通」「大網街道」「豊永橋」だった。国道の
               `国道N号` に当たる書き方は無い

  裏取り       国道で効くのは、県内で保証される番号が 459 のうち 10〜55 しか
               ないからである。都道府県道では県内に 759 番(北海道)や 432 番
               (兵庫)が実在し、保証集合が番号空間を埋める。リレーションが保証
               しない番号を落とすと、(県, 番号)13,247 組のうち 6,855 組(半分)が
               消える

残るリレーションと `ref` の和を採用する。番号は県の中でしか一意でないので、
路線の同一性は (県, 番号) の組である。所属県は prefectures.py が N03 の面で
決めた物を使う(#97)。

読むのは build/survey/{region}.json で、pbf を一度読んで都道府県道になりうる
way を県ごとに落とした物である(国道の build/cache に当たる)。あちらが残す集合は
意図して広く、`_link` と工事中も年報との差の内訳を測るために残す
(survey_prefectural.py)。判定はここだけが持つ。
...
# 都道府県道が持ちうる路線番号の上限。都道府県道のルートリレーションが保証する
# 番号の最大値は北海道の 1,180 で、次に現れる番号は 3,504 まで飛ぶ。1,181 から
# 3,503 に (県, 番号) の組は無いので、境目はここにしか置けない。
#
...
#
# 政令指定都市の市道が県道と同じ書式で番号を名乗っている形で、国道側で都市高速が
# `ref=4` を名乗るのと同じことが一段下の格で起きている。
MAX_REF = 1199

# 主要地方道の番号。主要地方道かどうかは政令で決まる法令上の区別で、OSM はそれを
# 直接持たない。持っているのは道の格と番号で、実測では primary の 95.5% が `ref`
# 100 以下、secondary の 99.5% が 101 以上である。
#
# 分けるのに使うのは番号である。格は道の作りであって路線の格付けではなく、格で
# 分けると東京都で 633.8 km の一般都道が主要地方道になる。県ごとに境目を実測して
# 動かす案は沖縄県でどの測り方でも壊れる。四つの案の突き合わせは PREFECTURAL.md
# にある。
#
# 42 県では 1〜100 が主要地方道である。残る 5 都道県では番号の振り方そのものが
# 政令の指定から外れるので、都道県ごとに書き下す。
#
#   北海道   主要道道は 101〜199 も持つ(道道106・118・121・123 号など)
#   東京都   特例都道の 301〜319 も主要地方道である(都道311号環八・318号環七)
#   福岡県   151 号だけが 100 を超える主要地方道である
#   大分県   57 号は主要地方道ではない。国道57号からの指定変更で番号が残った
#   沖縄県   政令の指定が飛び飛びなので、帯ではなく番号を並べる
#
# ここは法令の写しであって測って決めた値ではない。出どころは Wikipedia
# 「主要地方道」の路線番号の節と、各都道県の県道の一覧である。測って動かしては
# ならない。範囲は出典どおりに写す。北海道は「一般道道は 200 番台以降」と境目で
# 述べられているので 199 まで採用する。東京都は「301〜319番が特例都道である
# 主要地方道」と番号で述べられているので 319 で切る。次の特例都道の帯(401〜484)
# は一般都道で、320〜399 を主要地方道と読む根拠は無い。
#
...
def national_name_numbers(name: str | None) -> set[int]:
    ...
    全国で、候補のうち名前が `国道N号` と述べる way は 41 本あり、番号が `ref`
    とも一致するのは 3 本(0.17 km)である。うち 2 本は `primary_link` なので
    `admits_by_tag` が先に外し、この規則でしか落ちないのは静岡県の 1 本、0.03 km
    である。残る 38 本(4.9 km)は `name=石川県道22号金沢小松線;国道159号` の
    ように別の番号を述べる正当な重用で、番号を見ずに外すとこの 38 本を壊す。

    国道のリレーションに載っていることは使えない。候補のうち国道のリレーションが
    番号を与える way は 224 本あるが、番号が一致するのは 1 本だけで、残る 223 本
    (92.6 km)は青森県道3号 弘前岳鰺ケ沢線のように国道と重用する本物の県道で
    ある。「国道のリレーションに乗っている」ことと「国道として同じ番号を名乗って
    いる」ことは別の情報である(CASES.md 18)。
    """
...
def designations(doc: dict) -> tuple[set[int], set[int]]:
    ...
    番号を読む仕事は survey_prefectural.py が済ませてあり(`rel_refs`・
    `tag_refs`)、ここがするのは規則の当てはめだけである。同じ `ref` を二つの
    正規表現で読み直すと、片方が暗黙のうちに古くなる。
    """
...
def admits_by_tag(doc: dict) -> bool:
    ...
    トークンがあることを求める。`tertiary` は入れない。数字の `ref` を持つ way
    は primary 128,139、secondary 152,701 に対して tertiary 2,470 で二桁少なく、
    国道が primary を締め出したのと同じ理由で外せる。

    `_link` は認めない。ランプは台帳の路線延長ではなく、国道側の
    `is_national_grade` が `trunk_link` を認めないのと同じである。リレーションが
    抱えるランプは規則 a で入る。全国の `_link` 2,770 本・424.0 km のうち
    1,429 本・215.1 km がそちらで入り、残る 1,341 本・208.9 km がここで落ちる。

    工事中は認める。国道側が `construction=primary` を締め出したのは、数字の
    `ref` を持つ県道を裏取りへ流し込む裏口だったからで(北海道道39号が国道39号に
    なった)、こちらではそれこそが拾いたい物である。
    """
...
def rank_mismatch(region: str, grade: str | None, refs: list[int]) -> str | None:
    ...
    数えるのに使わなかったほうを見張りに回す。`primary` なのに番号が
    一般都道府県道、`secondary` なのに主要地方道、という way の数である。タグの
    誤りの数ではなく、二つの証拠が割れている量である。
    """
...
def refs_key(region: str, refs: list[int]) -> str:
    """絞り込みが読むキー。区切り文字で囲んだ (県, 番号) の並びである。

    国道の `,18,` に当たる物だが、県道18号は 47 本あるので、県の名前を番号に
    貼り付ける。貼り付ける県は way の所属県(`pref` 属性と同じ値)である。同じ
    ことを二度書いているように見えるが、片方は way の属性、もう片方は路線の
    同一性で、必要な形が違う。この形なら画面の絞り込みは国道と同じ 1 つの式の
    ままである。

        ["in", ",tokyo-318,", ["get", "refs"]]

    県を別の属性で絞ると式が二本になり、重用の照合(`,` で囲む理由)と県の照合で
    別々の書き方を覚えることになる。

    県は符号(13)ではなく地域名(tokyo)で書く。この repo が県を名指しする言葉は
    regions.py の地域名で、build/survey のファイル名も web/data/regions.json も
    `pref` 属性もそれである。ここだけ符号にすると県の名前が二通りになる。
    """
    return "," + ",".join(f"{region}-{r}" for r in refs) + ","


def check_keys(region: str, refs: list[int]) -> None:
    """キーが、その県の他の番号に誤って当たらないことを断定する。

    国道の `,4,` が `,14,` や `,400,` に当たらないのと同じ性質を、県を貼り付けた
    形でも保つ。紛らわしい相手(先頭が同じ番号、末尾が同じ番号、桁を伸ばした番号)
    を実際に当てて確かめる。式のほうを書き写して確かめても、
    確かめたことにならない。
    """
...
def build(region: str) -> dict:
    ...
    返すのは `arcs`(書き出すアーク)、`routes`(路線ごとの集計)、
    `meta`(数え上げ)、そして `arc_km`・`designated_km` である。後ろの二つを
    丸める前の値で返すのは、全国の合計をここから足すためである。`meta` の側は
    小数第 1 位に丸めてあり、47 県ぶんを足すと丸めの誤差が積み上がる。
    """
    ...
    for w in ways:
        ...
        if from_tag - from_rel:
            tag_added += 1
        # 重用するアークは種別の違う路線を同時に持ちうる。上の格を採用する。
        # compare_annual_report_pref.py が実延長を表12 と表13 に割るときと
        # 同じ扱いである。路線ごとの種別は下の `routes` が rank_of で持つ。
    ...
    # アークの実長の和。同じ道を何本の県道が指定していても 1 度しか数えない。
    # 年報の実延長 とは違い、あちらは国道と重用する区間を国道側へ寄せる。その
    # 寄せ方は compare_annual_report_pref.py が持つ。
    ...
    return {
        ...
        "arcs": arcs,
        "routes": routes,
        # 全国の合計はここを足す。`meta` の側は小数第 1 位に丸めてあるので、
        # 47 県ぶんを足すと 1 県あたり最大 0.05 km の誤差が積み上がり、
        # 全アークを足してから丸めた値と合わない。
        "arc_km": arc_km,
