...
裏取りが濾せるのは、信用する路線番号の集合が全国ぶんではなく 1 県ぶんである
あいだだけだからである。新しいのは後半で、地域を結合し、タイルを切り、地図が
全国になって初めて成り立つ問いを訊く。

パックの前に全地域を生成して検査し、失敗はまとめて報告する。最初の失敗で
止めれば、47 県のことを 1 回の実行につき 1 県ずつ知ることになる。それでも
パックは全県が通ったときにしか走らない。半分だけ生成した集合をアーカイブに
詰めた物は、完全な物と見分けが付かない。

県は互いに独立なので、同時に何県か走らせる。県ごとの段は自分の bbox の中だけを
見て、書く先も build/regions/<県>.* に分かれている。並列度の決め方と、その前に
何を直列で済ませておくかは REGION_JOBS を参照。
...
# 動いており、空いて見えるメモリを埋めればそれらが押し出される。「収まる最大」
# ではなく「速さがもう伸びない所」である。増やすときは --jobs で渡し、山も
# 一緒に測る。
#
# N13 のメッシュは、この並列に入る前に pack_n13.py が直列で用意しておく。1 次
# メッシュは県境をまたぐので、用意しないまま並列に入ると、隣り合う県が同じ符号を
# 同時に KSJ へ要求する。用意さえ済んでいれば読むのは mmap である。上の表で
