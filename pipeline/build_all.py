...
地域ごとの判定は意図してそのままである。裏取りが濾せるのは、信用する路線番号の
集合が 1 県ぶんであるあいだだけだからである。新しいのは後半で、地域を結合し、
タイルを切り、地図が全国になって初めて成り立つ問いを訊く。

パックの前に全地域を生成して検査し、失敗はまとめて報告する。最初の失敗で
止めると、47 県のことを 1 回の実行につき 1 県ずつしか知れない。それでもパックは
全県が通ったときにしか走らない。半分だけの集合を詰めたアーカイブは、完全な物と
見分けが付かない。

県は互いに独立なので同時に何県か走らせる。県ごとの段は自分の bbox の中だけを
見て、書く先も build/regions/<県>.* に分かれている。並列度と、その前に直列で
済ませておくことは REGION_JOBS を参照。
...
# 県を同時に何本まで走らせるか。
#
# 8 コア 16 スレッドの機械で 47 県を通しで測った(issue #103)。時間は秒、メモリは
# 全プロセスの合計である。RSS は mmap で共有する頁を本数ぶん重複して数えるので、
# USS(そのプロセスだけが持つ頁)も併記する。
#
#     本数    時間     RSS 計    USS 計
...
#
# **空きメモリを埋める本数を選んではいけない。**他のプロセスが同時に動いており、
# 押し出される。「収まる最大」ではなく「速さがもう伸びない所」である。増やす
# ときは --jobs で渡し、山も一緒に測る。
#
# N13 のメッシュは、並列に入る前に pack_n13.py が直列で用意しておく。1 次
# メッシュは県境をまたぐので、用意しないと隣り合う県が同じ符号を同時に KSJ へ
# 要求する。用意済みなら読むのは mmap で、1 本増やすごとに USS が 130〜150 MB
# しか増えないのは N13 を全員が同じ物理ページで見ているからである。本数に比例
# するのは Python と地域の GeoJSON だけである。
REGION_JOBS = 6
...
# いる。青森県は自分の進捗の行を出そうとして落ち、生成の失敗として報告された。
...
def outcome(label: str, code: int, out: str) -> list[str]:
    ...
    `failures()` が見つけた以上のことを示さない。新しい情報になるのは、FAIL 行
    が無いまま 0 でない終了コードが返ったとき(traceback や異常終了)だけである。
    それは FAIL 行ごとにではなく一度だけ報告する。
    """
    bad = failures(out)
...
def prefectural(judge: bool) -> None:
    ...
    国道と分ける理由は #100 にある。国道の 55.9 MB を県道を直すたびに上げ直さず
    に済み、タイル化のメモリが 2 回に分かれ、県道側が壊れても国道の地図は出る。

    判定が読む build/survey は、pbf を一度読む別の段(`mise run survey-pref`)が
    作る。ここが抱えると、国道だけを作り直したい手元に 2.5 GB の読み直しを
    強いる。無いときは飛ばし、何をすれば入るかを名指す。半分だけ詰めた
    アーカイブは揃った物と見分けが付かないので、47 県のどれかが欠けても飛ばす。
    """
    ...
    # アークが 1.9 倍あるぶんは埋まらない。実測では 1,024 MB で通り、896 MB で
    # 落ちる。
...
def positive_jobs(value: str | None) -> int:
    ...
    暗黙のうちに既定へ戻さない。`--jobs --no-pack` は値の抜けであると同時に、
    次のフラグを食べる指定でもある。`--jobs 0` を 1 に丸めるのも同じで、渡した
    本数と走る本数が違うまま気付けない。
    """
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=jobs) as pool:
        ...
        # 終わった順に書く。速い県が遅い県の後ろで待たされないので、進みが
        # 見える。何番目かは「幾つ終わったか」であって、名指しの順番ではない。
    ...
    # 書くのは、この後の decree.py と pack_web.mjs が読むのがこの
    # 索引だからである。
