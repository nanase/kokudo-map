...
verify.py が見るのは一度に 1 つの矩形である。だから二つのことが見えない。しかも
地図が全国を覆うようになると、効いてくるのはその二つである。

  番号が国の反対側へ漏れていないか
      裏取りは設計として地域ごとである(build_routes.py を参照)。それが切れ味を
      保っているのだが、同時に、国道372号が兵庫県——本来の場所——と長野県の
      両方に出たことを、地域ごとの実行では誰も見られないという意味でもある。
      長野県に出た実例は CASES.md 1 にある。見られるのは結合後のデータだけで、
      路線ごとにアークが実際にどこに在るかを訊いて確かめる。
...
# 答える。だから内訳が自分の延長に届かない行は、精度が粗いのではなく誤って答える。
# 語彙はここで述べ直さない。行が名指しできる区分は、地域ごとの生成物がアークを
# 分類した先である。
...
# --- 政令自身の起終点 ------------------------------------------------------
# 別表は決まった一覧である。一般国道 1 路線につき 1 行、計 459 行で、その一覧の
# ままであることがこの欄の眼目である。行に載る座標が作られることは無い——
# decree.py が返してよいのは、その路線が既に持っている端点だけである——ので、
# その主張は信用せずここで確かめる。「座標から路線のアークまでの距離」が 0 に
# なるのは、そのためである。
decree = meta["decree"]
rows = decree["routes"]
...
# 生成物の中でこれに反論できる物は無い——交差の表はこれ 1 つだからである。確かめ
# られるのは、これが交差の表であり続けることである。1 行に別々の実在する路線番号
# が二つ、同じ行は一度だけ、そしてアークを共有する組は決して入らない。最後の
# 一つが、この表が引こうとしている区別そのものである。同じアークに載る組は重用で
# あり、それは組み合わせ表が既に述べ、パネルも別の節で出している。両方に出る行は、
# 同じ事実を、食い違いうる二箇所で二度述べることになる。
...
if path.exists():
    ...
    # かつては全国の外接矩形の中心——北緯 34.93 度、東経 134.87 度、播磨灘の
    # 海上——を訊いていた。z14 のタイルは 2.4 km 四方なので、国道が 1 本も無い
    # 正方形を求めていたことになる。空のタイルは設計としてアーカイブに入らない
    # ので、正しいアーカイブに対して検査が落ちていた。
