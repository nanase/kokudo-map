...
隙間の長さが原因を仕分ける。生の数より、どの段に入るかが効く。
  ...
  50 m〜2 km   短い接続の欠落、バイパスの分岐、または工事中区間の境界(まだ
               繋がっていないのが正しい)。
...
def why_excluded(wid, tags, cache, corroborated):
    """この way が除外されていそうな理由を、大まかに推測する。

    build_routes.py の判定ルールを厳密には再現しない。
    names_a_closed_residential_road のような細かい規則は見ていない。当てはまる
    理由をすべて返し、どれも無ければ「unclear — investigate」を返す。
    """
    reasons = []
    c = claims(tags)
