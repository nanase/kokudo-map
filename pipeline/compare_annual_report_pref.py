...
二つある。
  ...
  2. 判定(#98)より先にこの段を立てた。判定が終わってから検証を作ると、判定に
     合わせた検証になる。だからここが読むのは生成物ではなく、
     survey_prefectural.py が OSM から測ったそのままの値である

台帳の定義をそのまま計算の骨にする。年報はどの表でも
実延長 = 総延長 - 重用延長 - 未供用延長 - 渡船延長 と定義する。総延長は路線
ごとの延長の和で、重用区間はその路線の数だけ数える。実延長は道そのものの長さ
で、重なりは一度しか数えない。
...
これは年報の定義そのものであって近似ではない。この置き方だと 表11 = 表12 + 表13
が計算のうえでも成り立つ。

種別は番号で分ける。主要地方道か一般都道府県道かは政令で決まる法令上の区別で、
OSM はそれを直接持たない。持っているのは道の格(primary・secondary)と番号で、
実測では primary の 95.5% が `ref` 100 以下、secondary の 99.5% が 101 以上で
ある。

分けるのに使うのは番号のほうである。格は道の作りを示すのであって、路線の格付け
ではない。二つが食い違う way は、下の「種別と道路の格」でそのまま報告する。
数えるのに使わなかった側を、代わりに見張りに使う。
...
を区別しないので、突き合わせる相手は両方の和である(annual_report.Prefecture)。

県境を跨ぐ way は、長さの過半が入る県に寄せてある(prefectures.py)。はみ出す
少数側の長さは県別の差にそのまま現れるので、県ごとの表にその量を並べる。差が
それより小さい県について、差を論じても意味がない。

県ごとの差は、そのままでは上下線分離を含んでいる。都市部の県はそこが大きい
(東京都の +25.6% は、引くと -1.8% になる)ので、引いた後の残りも並べる。県ごとの
原因を見分けられるのは、この列があるからである。
...
def is_national(doc: dict) -> bool:
    ...
    が国道だと述べていること(extract_pbf.is_candidate が答える問いで、答えは
    survey_prefectural.py が書いてある)である。

    タグの側は、その way が都道府県道の格を持たないときにだけ効かせる。
    is_candidate は国道の工事中区間を拾うために `highway=construction` を国道の
    格に数えるので、`construction=secondary`・`ref=34` の県道もそのまま通る。
    県道の工事中区間が国道として外れると、その延長は説明できない残りへ紛れ込む。
    primary・secondary は国道が使わない格なので、格が付いている way はタグでは
    国道にしない。
