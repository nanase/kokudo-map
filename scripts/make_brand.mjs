 ...
 * 画面のアイコン、リンクの共有カードである。
 *
 * 三つとも国道番号標識で、どれも web/shield.mjs の `SHIELD_PATH`(操作パネルと
 * ポップアップが使うのと同じ輪郭)から描く。ここで三角形を写せば、標識の形に
 * 二つ目の答えを持つことになる。
 *
 * favicon は SVG である。1 ファイルで済み、揃えるべき寸法も無く、標識は平らな
 * 形なので描いて失う物が無い。番号は載せない。16 px では番号は滲みでしかなく、
 * この地図が扱うのは番号の全部である。
 *
 * ホーム画面のアイコンと共有カードは PNG で、既にここにある Chromium で描く。
 * アイコンはカードを切り出した物ではなく、同じ題材を正方形に組み直した物である
 * (`iconHtml`)。三枚の標識が道の上に立ち、路線が合流するほど道の色が
 * 深くなる。本物の `shield()` を使うので、番号入りの標識の
 * 見た目に二つの答えは生まれない。
 *
 * どれも追跡する。数十 kB しかなく、標識か文言が変わったときにしか動かない。
 * 街路を種を渡した生成器で引くのはそのためで、二度走らせて
 * 同じバイト列にならなければ、走らせるたびに差分になる。
 *
 ...
 * social preview は 1280x640 を求め、SNS のカードが期待する 1200x630 と
...
/* 地の色。ホーム画面アイコンと共有カードの両方が使う。地図の画面自体は明暗を
 * 切り替えるが、これらは常にこの一色である。 */
const GROUND = '#0B1826';
...
/* (cx,cy) を中心に deg 度回した (x,y) を返す関数を作る。アイコンとカードは
 * 大きさが違う別々のキャンバスなので中心もそれぞれ持ち、同じ式を中心だけ変えて
 * 使い回す。 */
...
/* 国道の線・街路。区間ごとに 1 本、縦か横だけで、交わりは直角だけである。 */
...
/* ---------------------------------------------------------- home icon --- *
 *
 * ホーム画面に追加したときのアイコン(manifest.webmanifest が参照する)。favicon
 * (標識 1 枚)ではなく、共有カードの一番深い区間(単独指定→二重用→三重用)を
 * 正方形に切り出した絵にする。標識 1 枚では一般的な道路標識アプリに見え、重用
 * 区間という着想が伝わらない。
 *
 * 視点は左から右へ動くので、若い番号(73)を左・手前・大きく、大きい番号(215)を
 * 右・奥・小さく置く。道の色も揃え、若い番号だけの区間が青、合流するたびに濃く
 * なって右端が赤になる。標識は shield() をそのまま使う。太さ違いの縁をここで
 * 新たに作ると、番号付き標識の見た目に二つ目の答えを持つ。 */
if (!opt.card) {
  const ICON = 512;
  const iconRot = makeRot(ICON / 2, ICON / 2, TILT);

  /* 標識の並び。底辺(ICON_BASE)を揃えて道の上に立たせ、大きさで奥行きを
   * 出す。z は描く順で、奥(215)を先に、手前(73)を最後に描いて上に乗せる。 */
  const ICON_BASE = 350;
  const SIGNS = [
  ...
  /* 実際に書き出す大きさ(outSize)へは、512 単位で組んだ絵を CSS の scale で
   * 縮める。カードが CARD の組みを OUT の枠へ拡大縮小するのと同じやり方である。
   * safeZoneScale は maskable 専用の追加縮小(下記)である。 */
  const iconHtml = (
    outSize,
    safeZoneScale = 1,
...
<div class="icon"><div class="map">${mapSvg}</div>${pins}</div>`;
   ...
   * maskable の安全域は不要で、他の 2 枚と同じ絵をそのまま縮小する。 */
...
/* ------------------------------------------------------------------- card ---
 * カードは、この地図が存在する理由そのものを出す。複数の路線番号を持つひと
 * 続きの道である。二本の国道が本線に丁字路で入り、線は単独指定 → 二重用 →
 * 三重用と深くなる。深さは線の色、載る標識の数、標識の大きさの三つで同時に
 * 示し、最も深い区間を最も大きく描く。見る値打ちがあるのはそこだからである。
 *
 * 番号は 73・110・215 で、三つとも一般国道に無い番号である
 * (pipeline/build_routes.py の `VALID`)。だから実在の重用についての主張として
 * 読むことはできず、絵は測量ではなく構図として自由に組める。
 */
...
const ROAD_Y = 516;
const JOIN_A = 287; /* 南から。ここから二重用 */
const JOIN_B = 673; /* 北から。ここから三重用 */
...
/**
 * 標識の一枚一枚が、地図を倒した後どこに載るか。描く側も切り落としの上限を
 * 測る側もここだけを読む。位置の式を二度書くと片方だけ動かせてしまい、検査が
 * 読むのが写しなら、それは検査ではない。
 */
...
const GROUPS = [
  { refs: [73], x0: 205, h: 86 },
  { refs: [73, 110], x0: 403, h: 118 },
  { refs: [73, 110, 215], x0: 763, h: 158 },
];

/* 題字の左上。style.css ではなくここが持つ。下の隙間の計算が読む。
 *
 * 左の余白は、右端の標識(215)の外の余白と揃える。どちらも描いた字と縁で
 * 測ると約 74 px になる。題字の字面は箱より 9 px ほど内側から始まり、標識の
 * 白い縁は placements() の幅より 3 px ほど内側で終わるので、数字は揃わない。
 * 左だけ広いと、絵全体が右に寄って見える。 */
const TEXT_INSET = { top: 66, left: 65 };

/* ------------------------------------------------------------ 切り落とし ---
 * 求められた枠を覆うまで拡大してはみ出したぶんを切る作りなので、縦横比が
 * 1200:630 から離れるほど切る量が増え、どこかで題字か標識が欠ける。1080x1080 を
 * 渡すと横を 48% 切り、題字は「道マップ」になる。それでも終了コードは
 * 0 だった。
 *
 * 切ってよい量は、絵の中で最も外に出る物との隙間で決まる。横で最も外に出るのは
 * 右端の標識で、地図ごと 4 度倒してあるぶん倒す前より外へ出ている。手で 4.5% と
 * 書いていたのは倒す前の値だった。placements() に載る場所を訊いて測る。
 */
...
/* 両端で切るので、隙間の 2 倍まで許せる。縦も標識の実際の下端から測る。本線の
 * 高さ(ROAD_Y)は標識が載る前の値で、下端はそこより上にある。 */
const CROP_MAX = {
 ...
 * EISDIR で落ちると、1 秒ぶんの描画が無駄になる。確かめるほうを mkdir より先に
 * 置くのは、断る書き先のために空のディレクトリを残さないためである。
 *
 * 見るのは形だけで、書けるかどうかは見ない。親のディレクトリを作れなければ
 * mkdir が落ちるが、そこを越えて writeFileSync が権限で弾かれる書き先は、
 * 描き終えてから落ちる。実際に書いてみるほかなく、書いてしまえば確かめる
 * 意味が無い。 */
...
/* 追跡している 2 枚は 1200x630 でしか作らない。別の寸法でそこへ書くと、揃えて
 * 渡す決まりが防いでいるはずのこと(og.png を違う寸法で潰す)がそのまま
 * 起きる。 */
if (
  opt.card &&
