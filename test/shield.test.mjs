/* 地図が路線を名指しするところで描く国道番号標識。SHIELD_PATH は実物の標識の
 * 比率を写した物で計算した物ではないので、導き直せる式はありません。確かめる
 * 値打ちがあるのは、閉じた正しい形のパスであり続けることと、点が自分の viewBox
 * の中に収まっていることです。編集で壊れても(桁の脱落、符号の誤り)、
 * そうでなければブラウザで形が崩れて初めて分かります。
 */
...
/**
 * `M`・`c` の区間が名指しする点(端点と、`c` の二つの制御点)をすべて返します。
 * `c` の相対の差分は `M` の始点から辿って解きます。三次曲線はこれらの点の
 * 凸包から出ないので、点を検査すれば曲線そのものを抑えられます。
 */
function anchors(d) {
  const points = [];
  let [x, y] = [0, 0];
 ...
 * ここでも導き直せる式はありません。確かめることはおにぎりと同じで、違うのは
 * コマンドが絶対の M・L・C であることと、枠が HEX_VIEWBOX
 * であることだけです。 */
describe('HEX_PATH', () => {
  /** 絶対の M・L・C が名指しする点をすべて。三次曲線はこの四点の凸包を
   * 出ません。 */
  const hexPoints = (d) => {
    const points = [];
  ...
  test('おにぎりと違って横長である', () => {
    // 実物の標識の比です。上下が水平な辺、左右が頂点なので、幅が高さを
    // 超えます。
    const { w, h } = extent();
  ...
  test('縁を含めた外径の高さがおにぎりと揃う', () => {
    // 二つの標識は同じ行に並びます。高さを CSS の .shield 一つで
    // 決めているので、外径の高さがずれると、並べたときに片方だけ小さく
    // 見えます。枠の高さと縁の太さは別々に決められるので、
    // 揃っていることをここで押さえます。
...
describe('hexShield', () => {
  ...
  test('県名は絵として描かない', () => {
    // 操作パネルでの実寸は高さ 30 px です。その大きさでは県名は形になりません。
