...
describe('deepest', () => {
  ...
  test('最も深い重用のアークを選ぶ', () => {
    // 交差点や立体交差では複数のアークが 1 画素の下に重なる。返る順はタイル
    // 次第で、福岡の四重用では 国道202号 が単独で報告された。
  ...
  test('n が文字列で来ても数として比べる', () => {
    // ベクタタイルの属性は文字列で届くことがある。文字列比較だと '10' < '9'。
    expect(
...
describe('popupHTML', () => {
  ...
  test('対応表に無い値はそのまま出す', () => {
    // 生成側が新しい種別を足したとき、空欄になるより生の値が見えたほうがよい。
  ...
  test('長さは区間長であって延長ではない', () => {
    // クリックした 1 本の way の長さである。延長と書くと 国道4号 が 0.13 km に
    // 見える。
