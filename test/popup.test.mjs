/* 押したアークのポップアップ。1 画素の下に複数が重なるときその押下がどの
 * アークを指すのか、そして OSM の文字が文字のままページへ届くかどうか。この
 * 二つが、画面上は問題なく見える形で間違っていたことがあります。
 */
...
describe('deepest', () => {
  ...
  test('n が文字列で来ても数として比べる', () => {
    // ベクタタイルの属性は文字列で届くことがあります。文字列比較だと '10' <
    // '9'。
    expect(
...
describe('prefPopupHTML', () => {
  /* build_prefectural.py が書く属性です。国道と違って `updated` を
   * 持ちません。 */
  const parc = (over) => ({
    id: 173704782,
    pref: 'nagano',
