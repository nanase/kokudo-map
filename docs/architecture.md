...
ゾーン設定は項目ごとに個別に書く。当てるのは `browser_cache_ttl`・`cache_level`・`sort_query_string_for_cache` の 3 つだけである。`development_mode` は記録するが当てない。設定ではなく一時的な状態であり、誰かが調査のために有効にしている最中に書き戻せば断りなく無効化してしまい、記録が古ければ望まれないまま有効化してしまう。どちらもこちらが決めてよいことではない。
