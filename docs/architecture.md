...
| `development_mode` | しない | 設定ではなく一時的な状態である |
| `sort_query_string_for_cache` | しない | このゾーンの契約では API が編集を拒む(code 1015) |

`development_mode` を書き戻すと、誰かが調査のために有効にしている最中に断りなく無効化してしまう。逆に記録が古ければ、望まれないまま有効化してしまう。どちらもこちらが決めてよいことではない。

`sort_query_string_for_cache` は読み取り専用で、同じ値を送っても拒まれる。試みれば、そこまでの書き込みだけが済んだ状態で落ちる。当てられない物は初めから当てない。
