...
| `web/data/pref/summary.json` | 都道府県道の全国集計(対象アーク・延長・重用アーク・路線数)。「この地図について」を開いたときに読む |
...
線の形は変えません。簡略化の閾値は国道と同じ 3 のままです。それでも z0-7 の最大タイルは 2,322 kB から 1,258 kB になります。国道の低ズーム最大 2,232 kB に対して 56% です。実測は [結果](results.md#配信データ) にあります。
...
`pref/summary.json` はこの分け方の例外に見えますが、47 県を積み上げた 4 個の数(対象アーク・延長・重用アーク・路線数)だけを持つ数十バイトのファイルで、県別 meta のような路線ごとの内訳は持ちません。「この地図について」の都道府県道側の欄が使う物で、県を選んだときの遅延取得とは別に、開いたときだけ 1 度取ります(初期表示では読みません)。
...
`nanase.cc` ゾーンのキャッシュ関連設定を `pipeline/cf-cache.json` としてリポジトリに持つ。ファイルが正であり、ダッシュボードを開かずに取得・比較・適用ができる。

```sh
mise run cf-cache        # 取得して pipeline/cf-cache.json へ書き出す
mise run cf-cache-diff   # ファイルと実際の設定の差分を出す
mise run cf-cache-apply  # ファイルの内容を実際のゾーンへ当てる
```
...
適用は本番ゾーンを書き換える。試せる別のゾーンは無い。

**Cache Rules は丸ごと置き換わる。ファイルに無いルールは消える。** Rulesets API の entrypoint が配列ごとの PUT だからである。ダッシュボードでルールを足した人がいて、それが記録から漏れていると、当てた瞬間に黙って消える。実際に `.mjs` のルールが記録から漏れていたことがある。

そのため関門を 3 つ置いている。

1. `--apply` を明示しないと、何があっても書かない
2. 追加・変更・**削除**を人が読める形で表示する
3. 合言葉(`apply`)を打ち込むまで書かない

消えるルールがあるときは、`mise run cf-cache-apply` はそこで止まる。進めるには `-- --allow-delete` を添える。合言葉も `delete` に変わる。いつもの手つきで打ち抜けないようにするためである。

```sh
mise run cf-cache-apply                    # 消えるルールがあれば止まる
mise run cf-cache-apply -- --allow-delete  # 消えると分かったうえで進める
```

計画を見るだけなら、合言葉を打たずに Enter を押せばよい。表示だけして中止する。

合言葉を待っている間も、ダッシュボードは開ける。そこで足されたルールは、表示にも `--allow-delete` にも掛からないまま消えてしまう。そこで、書く直前にもう一度取り直し、見せた計画がまだ本当かを確かめる。変わっていれば何も書かずに終える。もう一度実行して、新しい計画を見ることになる。

ゾーン設定は項目ごとに個別に書く。記録する 4 項目のうち、当てるのは 2 つだけである。

| 項目 | 当てる | 理由 |
| --- | --- | --- |
| `browser_cache_ttl` | する | ブラウザ保持期限の不具合の出どころ。ずれたら直したい |
| `cache_level` | する | 配信の挙動を決める |
| `development_mode` | しない | 書けるが、こちらが決めてよいことではないので当てない |
| `sort_query_string_for_cache` | しない | 書けない。API が編集を拒む(code 1015) |

当てない 2 つは、当てない理由が違う。混ぜて読まないこと。

`development_mode` は API が編集を許す(Cloudflare も `editable=true` と答える)。当てないのはこちらの判断である。書き戻すと、誰かが調査のために有効にしている最中に断りなく無効化してしまう。逆に記録が古ければ、望まれないまま有効化してしまう。どちらもこちらが決めてよいことではない。

`sort_query_string_for_cache` はそもそも書けない。このゾーンの契約では読み取り専用(`editable=false`)で、同じ値を送っても拒まれる。試みれば、そこまでの書き込みだけが済んだ状態で落ちる。当てられない物は初めから当てない。

どちらも記録からは外さない。ずれていること自体は知りたいので、`mise run cf-cache-diff` には出る。

当てるのは、計画に「変わる」と出た項目だけである。ゾーン設定は項目ごとに別の呼び出しなので、要らない 1 本が失敗すると、そこまで書けた状態で落ちる。`sort_query_string_for_cache` が実際にそれで落ちた。

ルールの `id` は記録に入れない。Cloudflare が決める揮発的な値で、記録に入れると設定を変えていなくても差分が出る。当てるときだけ実態から引き、`description` で突き合わせて添える。添えないとルールが毎回作り直され、Cloudflare 側の履歴が毎回「全消し・全作成」に見える。

当てたあとは、取り直してファイルと一致することをその場で確かめる。一致しなければ終了コード 1 で落ちる。見るのは当てた分だけである。当てない 2 項目まで含めて比べると、`development_mode` が有効な最中は、書き込みが全部成功していても落ちてしまう。ずれていればその旨だけを添える。
...
ただしこれは読みであって確かめてはいない。外すかどうかはここでは決めない。外すなら、`.mjs` が `DYNAMIC` に戻らないことと、Lighthouse の警告が再発しないことまで確かめる必要がある。
