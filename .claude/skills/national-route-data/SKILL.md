...
所属都道府県は矩形では決まりません。番号が県の中でしか一意でない都道府県道を扱う前に、面で決める必要があります。国土数値情報 N03 の行政区域から決めていて、理由と実測は `pipeline/prefectures.py` にあります。地域の分け方は矩形のままです。そこを面にすると裏取りの保証集合が変わり、国道の判定そのものが変わります。
...
way 自身が主張する番号は、同じ地域の国道リレーションが独立に保証している番号だけ採用します。この絞り込みが効くのは、保証集合が地域ぶんしかないためです。
...
全国をひとまとめに判定すると保証集合は 459 に近づき、何も濾さなくなります。国道 372 号は京都・兵庫に実在するので、長野県道 372 号がそのまま国道 372 号として復活します。過去の実例は [CASES.md](CASES.md) の 1 番と 2 番です。

`verify.py` は各地域の保証集合が 459 の三分の一未満であることを確かめます。`verify_national.py` は路線ごとの実在範囲を見て、番号が遠くの県に漏れていないことを確かめます。
...
| `web/data/pref/index.json` | 全国の県と番号だけの索引。14.4 kB。「道路を選択」が読む |
