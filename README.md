...
push と pull request では GitHub Actions が前者を回す。静的検査、単体テスト、仕様の検査、Python の構文、資材が組めること、共有用の札の差し替え先が残っていることを見る。判定の正しさと実描画は 2.5 GB の pbf と 47 地域ぶんの生成を要求するので、手元でしか答えられない。`main` への配信は検査を通らなければ走らない。

単体テストが守るのは、この地図の存在理由そのものである。組み合わせ表を路線ごとに開く読み方（`routesOf`）と、選択が触れる行を一度ずつ足す読み方（`statsFor`）を取り違えると、重用区間を路線の数だけ数えてしまう。それは地図が隠すのをやめさせたい数そのものである。実際に取り違えを作り込んで、テストが落ちることを確かめてある。
