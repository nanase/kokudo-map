 ...
 * ライセンスが付いてくる。独立したモジュールにしてあるのは、
 * scripts/make_brand.mjs が favicon と共有画像を同じパスから描くためである。
 * 輪郭の写しを二つ持つと片方だけずれていく。
 */
...
/**
 * ヘキサの輪郭。上下が水平な辺、左右が頂点の六角形で、角は丸い。素材のパスを
 * 1.1961 倍し、`HEX_VIEWBOX` の中央へ寄せた。倍率は縁を含めた外径の高さが
 * おにぎりと 428.02 で一致するように決めた。四方の余白は左右 8、上下 9.5 あり、
 * 縁が外へ滲んでも切られない。素材の六角形は上の辺が下の辺より 1.6 単位長い
 * 非対称だが、直さずに写す。0.4% は高さ 30 px で 0.1 px に足りない。
 */
