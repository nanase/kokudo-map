 ...
 * 指す先の真下に来る。
 *
 * 0 のときは行ごと出さない。丸める前の値では判定しない。0.04 km は fmtKm で「0.
 * 0 km」になるので、丸める前で判定すると「0.0 km」のまま出る。重用区間(route.
 * conc_km)は持たないときも「なし」と書くが、旧道は書かない。 */
 ...
 * 初めて埋まる(区分別は #58、起終点は #59、旧道の距離は #84)。欄が無ければその
 * 欄ごと出さない。
 *
 ...
 *
 * 「この路線だけ表示」は国道と同じ物である(onlyButtonHTML)。操作面に
 * 都道府県道の節が無いので、選んでいることを示す場所も解除する操作もこの
 * ボタンのほかに無い(#109)。重用の但し書きは「国道マップについて」が持つ(panel.
 * mjs の prefConcurrencyHTML)。パネルは 1 路線の数を出す場所で、数え方を
 * 説明する場所ではない。
 *
