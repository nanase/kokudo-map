 ...
 * 割合、政令上の起終点)を述べる場所は無かった。ポップアップの標識を押すと開く。
 *
...
/**
 * 政令上の起点・終点。national.meta.json がその欄を持たなければ空である。
 ...
 * 拾った番号は後の区画へ落とさない。
 *
 * 欄を持たない meta では、その区画ごと空になる。`crossings` は後から入った
 * 欄なので、古い web/data を配ったままでも壊れない。都道府県道の県別 meta は
 * `shared_termini` を持たない(全国 1 枚の起終点の表が無い)ので、その
 * 区画はいつも空になる。
 *
 * `system` は系統の呼び名で、区画の見出しに入る。`compare` は路線の並べ方、
...
export function relatedRoutesOf(
  ...
  return [
    ...
    { key: 'termini', label: `起終点が重なる${system}`, refs: sorted(ends) },
    { key: 'cross', label: `交差する${system}`, refs: sorted(cross) },
  ].filter((g) => g.refs.length);
}

/**
 * いま見ている路線が属する群。県境で続く路線を束ねた物で、県別 meta の
 * `continuations` が持つ(pipeline/pack_web_pref.mjs、issue #155・#162)。
 * 番号は県境で変わることがある。愛知県道156号は岐阜県道390号として続く。
 *
 * 路線は高々一つの群にしか入らない。群は県境で端点を共有する組の連結成分なので、
 * 二つの群に同時に入れば、それは一つの群である。
 *
 * 群を持たない県の meta には欄そのものが無い。`crossings` と同じく後から入った
 * 欄なので、古い web/data を配ったままでも区画が出ないだけで壊れない。
 */
 ...
 * 都・道・府・県 だけを並べて数を付ける。
 *
 * 「複数県にわたる」とは書けない。567 群のうち 136 群(24.0%)は「県」だけでは
 * 言えず、京都府と大阪府だけで閉じる 14 群には県が 1 つも入らない。「複数の
 * 都道府県にわたる」は正しいが 16 字あり、区画の見出しの 1 行に収まらない。
 *
 * 渡すのは群の全員の県名(`web/data/regions.json` の `label`)である。数える前に
 * 重複を落とす。**路線の数ではなく県の数を数える。** 同じ県が 2 度入る群は
 * 実在し、県境を 2 度またぐ路線がそれを作る。秋田県道131号・秋田県道210号・
 * 山形県道210号 は 3 本で 2 県である(issue #162)。数えずに本数を出すと、
 * 2 県の群が `3県` と名乗る。
 *
 * 例外表を持たない。北海道は群に入らないので `道` は出ないが、規則としては
 * 同じに扱う。C 回の絞り込みアイコンの `title`(`3都県まとめて表示`)もここを
 * 読む。同じ文言を二箇所で組むと片方が暗黙のうちに古くなる。
 */
export function continuationCountOf(labels) {
  const uniq = [...new Set(labels)];
  const tails = new Set(uniq.map((l) => l.slice(-1)));
  const kinds = [...SPAN_ORDER].filter((c) => tails.has(c)).join('');
  return `${uniq.length}${kinds}`;
}
 ...
 * 標識の番号と同じように自前で配る。
 *
 * 隣の絞り込みアイコンと同じ svg にしてある。textLength は shield.mjs の番号と
 * 同じ用途で、どの書体でも枠から食み出させない。21 は EB Garamond の W の
 * 送り幅(font-size 22px で 20.97px)で、この書体では伸び縮みしない。20 だった
 * ころは 20/24 に潰れ、細い斜め線がさらに細くなっていた。 */
...
/* 絞り込みアイコン。「番号で絞り込み」と同じく他を落として一つにする操作な
 * ので、同じ形にする。 */
const ONLY_ICON =
  '<svg viewBox="0 0 24 24" aria-hidden="true">' +
...
export function onlyButtonHTML({
   ...
   * 置き場所が述べる。見出しの絞り込みアイコンの隣には標識と路線名があり
   * 「長野県道1号だけを表示」と読め、区画の絞り込みアイコンの隣には相手の
   * チップがあり「この 3 本を表示」と読める。同じ絵であるほうが、同じ操作だと
   * 分かる。
   *
 ...
 * 短い。数が多い区画では 30 を超える(国道 4 号の交差は 31 路線)。小さいほうの
...
/* 群の相手のチップ。重用・交差の区画はヘキサだけを並べるが、あちらの相手は必ず
 * 同じ県で、番号だけで名指せる。この区画の相手は別の都道府県であり、538 群の
 * うち 525 群は番号まで同じである。「1」の隣に「1」を並べても何も述べて
 * いないので、県名を添えたチップにする。ここに並ぶのは多くて 3 枚(4 県の群が
 * 1 件)で、交差の 31 本とは事情が違う。
 *
...
/**
 * 複数の都道府県にわたる路線の区画。長野県道1号・愛知県道1号・静岡県道1号は
 * 飯田富山佐久間線 として県境で直接つながっているが、県別の数だけでは
 * それを述べられない(issue #155)。
 *
 * 関わりの区画(重用・交差)より前に置く。県境の向こうまで同じ道であることは、
 * 一点で交わることより強い関わりである。パネルの中でこの区画だけが地の色を持つ
 ...
 * チップからは外す。自分の標識は見出しに出ている。
 *
 * 路線名は取れないことがある(567 群のうち 41 群)。そのときは行ごと出さない。
 * 欄そのものが無い形で来るので、`name` の有無で分かる。番号が県境で変わる群は
 * 名前も変わることが多く、県ごとに別の路線名を認定している以上それが普通で
 * ある(issue #162)。
 */
const continuationHTML = (key, cont, prefLabels, selected) => {
  // 県が分からなければ自分の鍵も作れない。外せない自分がチップに並ぶくらいなら
  // 区画ごと出さない。「だけを表示」が region を要求するのと同じ事情である。
  if (!cont || !key) return '';
  ...
  return (
    ...
    // 絞り込みアイコンは行の右端に置く。何を絞るのかは、隣に並んだチップが
    // 述べる。押した状態は枠の色も担う。絞り込みアイコンは 15px しかなく、
    // 地図の上でパネルは 0.62 倍に見えるので、そこでは 9px になる。効いて
    // いることを述べるのは面のほうである。
    '<div class="cont-row">' +
 ...
 * (都道府県道には全国 1 枚の起終点の表が無い)だけである。
 *
 * 「この路線だけ表示」は国道と同じ物である(onlyButtonHTML)。操作パネルに
 * 都道府県道の区画が無いので、選んでいることを示す場所も解除する操作もこの
 ...
 *
 * 国道に無い区画が一つある。県境で番号が変わらずに続く路線を束ねた区画で、
 * `continuation` は continuationOf() の結果、`prefLabels` は 県 → 県名 の
 * 対応表である。国道の番号は全国で一意なので、あちらに同じ区画は要らない。
 *
 * 押した状態は二つある。`selected` は見出しの絞り込みアイコン(この 1 本だけ)、
 * `groupSelected` は区画の絞り込みアイコン(群をまとめて)である。排他では
 * ない。群を表示している間に見出しの絞り込みアイコンを押せば 1 本に絞れる。
 * 狭いほうが後から勝ち、その結果は
