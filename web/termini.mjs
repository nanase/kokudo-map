/* 起点・終点を GeoJSON にする。数千点しかなく操作パネルにも出ているので、
 * タイルに混ぜず素の GeoJSON で置く。ビルドが渡す二つの一覧(1 路線の起終点と、
 * 複数の路線が一緒に始まる・終わる地点)を一つのソースから描き、`shared`
 * フラグで見分ける。`refs` はアークと同じ区切り文字で書くので、
 * 1 路線に絞る式がそのまま効く。
 */
