 ...
 * 造成地のような明るい面で、薄い灰は 2.50 と 1.71 しか出ないが、濃い灰なら
 * 6.93 と 4.74 出る。暗い森では逆転するが、緑自身が 3.34 あって足りる。
...
export function prefLineLayers(basemap = DEFAULT_BASEMAP) {
  return [
    ...
    },
    {
      ...
      // 無かった。現道では灰を透けさせない。透かすと輪郭を持たせた意味がその
      // 分だけ戻る。旧道は線と同じく formerOpacity() で薄くする。
