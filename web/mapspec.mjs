 ...
 *
 * `PREF_GENERAL` は元々 #8CBF4A だったが、淡色地図の上で明るすぎて沈み、特に
 * 広域(z9 前後)ではほとんど判別できなかった(白地に対するコントラスト比
 * 2.17:1)。単に暗くする案・主要地方道 `PREF_MAJOR` と同じ明るさまで沈める案も
 * 試したが、実地図での比較の結果、主要地方道(色相 142°、青緑寄り)との色相の
 * 距離を広げる方向(黄色寄りの緑、色相 70°)を選んだ——明暗差が縮むよりも、
 * 細い線で色相が離れているほうが二つの格を読み違えないためである(コントラスト
 * 比は 2.94:1 に改善)。
 */
