...
// `expressway`(highway=motorway: 第二神明道路、神戸淡路鳴門自動車道など)は走れる
// 車道なので破線の区分には入らないが、平面交差が無く高速道路としての路線番号を
// 別に持つ別種の道なので、`roads` に畳まず自分の層を持つ。工事中や点線国道とは
// 独立に消したい読み手がいる。
...
/* 下地図は、上に載る国道に触れないまま別の地理院タイルへ差し替えられる。タイルの
 * 切り方も出典も同じで、違うのは絵だけである。三つともラスタ層として描くので
 * (baseStyle)、切り替えは表示・非表示の反転であって、ソースの作り直しではない。 */
 ...
 * 明るい配色では白に、暗い配色では黒に近づき、不透明度を上げると人によって明るく
 * も暗くもなる。`raster-brightness-max` は混ぜる前にタイルの画素を縮めるので、
 * 下げればどの地の上でも暗くなる。不透明度はこのサイトがずっと配ってきた 0.82 の
 * ままにし、`light` は今までの絵を保つ。濃い側の二段だけが明るさを動かす。 */
 ...
 * 区切りの 11 字、2 ファイル約 5 kB である。scripts/make_glyphs.mjs が Noto Sans JP
 * から作る。以前は国土地理院のデモ用の配布元を指していた。他人の Pages サイトで、
 * 消えればラベルも全部消える。 */
 ...
 * 残るのは選んだ道路だけになる。国道を選べば都道府県道は消え、都道府県道を選べば
 * 国道が消える。以前は選んだ系統の側だけを絞っていたので、国道を 1 本選んだ
 * 地図に都道府県道 13,234 組が網として残り、何を選んだのか読めなかった。詳細
 * パネルの「この路線だけ表示」だけが系統トグルを裏で倒してそれを避けており、同じ
 * 選択が押した場所によって違う絵になっていた。
 *
 ...
 *
 * 重用は道の性質であって選択の結果ではない。18 号と 117 号を持つアークは、両方を
 * 選んでいるかどうかに関わらず重用区間である。
 *
...
/**
 * ポップアップが説明しているアークの下に敷く影。OSM の way id はそれだけでアーク
 * を一意に指す(ビルドの重複排除もこれをキーにする)が、共有の式も畳み込む。選択で
 * 地図から外れたアークが、影だけを残さないためである。
 */
...
/**
 * ズームで補間する線の太さ。`zoom` の式は最上位の `interpolate`・`step` の直接の
 * 入力にしか書けないので、路線ごとの掛け算は補間の出力の側に置く。
 */
  ...
  return [
    {
      ...
      // drop-shadow は効かないので、影も線として持つ。道より太く、ぼかし、黒く、
      // 他のすべての下に描く。押されるまで何も描かず、押されたら app.js が way id
      // に絞る。影は白い縁取り(道より 2.6 px 太い)の外へ出る必要がある。+9 px、
      // ぼかし 5 px では縁取りの外に残る輪が細く淡すぎて見えなかった。
      id: 'picked',
      type: 'line',
      source: 'routes',
    ...
    },
    {
      ...
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
        // 重用は単独指定の上に描く。並べ替えのキーが無いとタイルの順のままになり、
        // 四重用が単独指定の下に埋もれる。押したときもそう扱われ、福岡の四重用が
        // 国道 202 号の単独指定と報告された。`line-sort-key` は昇順に並べて最後に
        // 一番大きい物を描くので、`n` を渡せば最も深い重なりが上に来る。
        'line-sort-key': ['get', 'n'],
      },
      paint: {
        'line-color': colorByN,
    ...
    {
      // 高速道路として指定された国道(highway=motorway)。走れる車道なので `roads`
      // と同じ体裁で描くが、点線国道や工事中と同じように単独で消せるよう層は分ける。
    ...
    },
    {
      ...
      layout: {
        ...
        // 符号を反転すると最も深い重なりから置かれ、ふつうの地図が丸めるラベルが
        // 最後に諦められる。
 ...
 * 49.5%、山間部で 71.4% が純白で、白の縁取りは輝度比 1.01 で何もしていなかった。
 * 白地に対して 3:1 に届く暗さと、写真の上で沈まない明るさの両方を一色に求める
 * ことになる。縁取りを薄い灰にすると輪郭がそちらに移り、格の二色は色相の間隔で
 * 選べる。`PREF_GENERAL` の黄は白地に対して 1.24 しかないが、縁取りがあるから
 * 読める。地図帳の黄色い一般県道と同じ成り立ちである。CIELAB 上の距離は格の
 * 二色どうしで 68.2、縁取りとの間で 73.2 と 101.8 ある。
 *
 ...
 * 明るい緑は輝度比が 1.29 しかないが、灰に彩度がほとんど無く緑の彩度が 68 ある
 * ので CIELAB 上の距離は 73.2 あり、縁として読める。
 *
 * 写真の下地図のときだけ、主要地方道の縁取りを濃くする。緑が弱くなるのは雪や
 * 造成地のような明るい面で、薄い灰は 2.50 と 1.71 しか出ないが、濃い灰なら 6.93
 * と 4.74 出る。暗い森では逆転するが、緑自身が 3.34 あって足りる。一般都道府県道
 * の黄は写真のどの面に対しても明るいので、薄い灰のままでよい。
 */
...
/* 都道府県道は国道の下に敷く。同じ太さで引くと、上に載る国道が県道の網に埋もれる
 * (アークは国道 151,004 に対して 290,529)。国道の ZOOM_STOPS のおよそ 0.65 倍を
 * 基準にし、格で細み分ける。 */
...
// 重用は色ではなく太さだけで表す。国道の N_MULT より刻みが浅いのは、県道の重用が
// 二重までにほぼ収まるためである。290,529 アークの内訳は単独 276,433、二重
// 13,424、三重 646、四重 26 で、三重以上は 0.23% しかない。
...
/**
 * 影を敷く層の id。国道の `picked` と同じ役目だが、一つの層で兼ねられない。キーは
 * OSM の way id で、国道と重用する都道府県道のアークは二つのアーカイブに同じ id
 * で入っているため、一つの層にすると県道を押したときに国道の線が光る。
 */
...
/** 押されたら答える都道府県道の層。破線の層も押せる。工事中の区間もポップアップに
 *  出す情報を持つ。縁取りとラベルは押しても路線を指さない。 */
 ...
 * boot)、国道の見え方を変えない。
 *
 * 国道と違って区分ごとに層を分けない。走れない区分は 290,529 アーク中 1,037 しか
 * なく、「点線国道」「海上国道」のような区分ごとの呼び名も都道府県道には無い。
 * 破線であること自体が「走れない」の印なので、色は格のままにして 1 層にまとめる。
 * `line-dasharray` はデータ駆動にできないが `line-color` はできるので、分ける
 * 必要があるのは破線の形だけである。
 *
 * `basemap` は縁取りの色だけに効く(`prefCasingColor`)。省くと既定の下地図の値に
 * なるので、層があることだけを問う側は引数を渡さない。
 */
export function prefLineLayers(basemap = DEFAULT_BASEMAP) {
  return [
    ...
    {
      // 灰の縁取り。国道の `casing` と同じ役目で、破線の区分には敷かない。国道と
      // 違って白ではなく薄めもしない(PREF_CASING)。淡色地図の半分以上は純白で、
      // 白を 0.85 で引いても輝度比は 1.01 にしかならず、輪郭が無かった。灰は
      // 透けさせない。透かすと輪郭を持たせた意味がその分だけ戻る。
 ...
 * そこで見た目と別に透明な層を重ね、そちらだけを太らせて当たり判定に使う
 * (app.js の wirePopups)。透明な層は不透明度 0 で描かれるだけで、ソースの追加
 * 読み込みは無い。`queryRenderedFeatures` に矩形を渡す手は層を増やさないが、
 * `mouseenter`/`mouseleave` は点でしか判定しないので、カーソルの形と押せる範囲が
 * ずれる。透明な層なら同じ id を hover にも click にも使えて、二つが揃う。
 *
 * 太らせる量は固定のピクセル数である。狙いやすさは画面上の距離で決まり、地図の
 * 縮尺では決まらない。値は市街地の込み合った場所で、隣の道路を誤って拾わない
 * ことを確かめて決めた。 */
const HIT_ADD = 10;
