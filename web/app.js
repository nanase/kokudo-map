 ...
 * 現れないので地図の側に置き、区画を分けて一つに収める。ボタンを分けると、
 * どちらを押すか毎回考えることになる。
 */
...
async function boot() {
   ...
   * 操作パネルに都道府県道の区画は無いので(#109)、絞っていることを示す場所も
...
async function openPrefDetail(key) {
  ...
  // 県境で番号が変わらずに続く路線の群(#155)。相手は別の県なので、チップに
  ...
  detailBody.innerHTML = prefDetailHTML({
    ...
    // 区画の絞り込みアイコンの押した状態。見出しの絞り込みアイコンと同じく、
    // 書き込む直前に毎回聞く。県別 meta を待つあいだにも選択は変わりうる。
    groupSelected: cont
 ...
 * 一致する場合だけ開く(#155)。区画の絞り込みアイコンが作れる形はそれだけ
 * で、押した人にとっては 1 つの選択だからである。開かないと、県道が 3 本だけ
 * 出ている理由も解除する口も画面から消える。
 *
...
document.addEventListener('click', (ev) => {
  ...
  if (only) {
    ...
    // 同じ経路になる。
    // 区画の絞り込みアイコンは群を名指す(#155)。押した路線のパネルは開いた
    // ままにする。解除する口はこの絞り込みアイコンのほかに無い。
